...
# 克隆GitHub仓库到临时目录
#git clone https://${GITHUB_USERNAME}:${GITHUB_PASSWORD}@${GITHUB_REPO} $TMP_DIR
sudo git clone https://github.com/eric010101/be $TMP_DIR

# 检查GitHub上是否有证书
if [[ -f $TMP_DIR/fullchain.pem && -f $TMP_DIR/privkey.pem ]]; then
    echo "从GitHub下载证书..."
    sudo mkdir -p $CERT_DIR
    sudo mv $TMP_DIR/fullchain.pem $CERT_DIR/fullchain.pem
    sudo mv $TMP_DIR/privkey.pem $CERT_DIR/privkey.pem
else
...
# 设置证书权限
sudo chown root:root $CERT_DIR/fullchain.pem $CERT_DIR/privkey.pem
